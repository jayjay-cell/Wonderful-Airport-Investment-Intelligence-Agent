"""assess_airport_opportunity tool: the full seven-dimension opportunity
assessment for one airport. This is where all of core/'s classification,
...
from core.models import Evidence, InvestmentFocus, Level, ObjectiveType, OpportunityAssessment
...
def assess_airport_opportunity(
    ...
    research: bool = False,
    include_bts: bool = True,
) -> dict:
    """include_bts=False produces a fast, FAA-only screening assessment
    (Demand Level only — no BTS-backed pressure/bottleneck data). Used by
    rank_airports's pre-screen pass; never the default for a direct
    single-airport question, where the full picture should always be used.
    """
    try:
        ...
        return {
            ...
            "detail": f"Unrecognized investment_focus {investment_focus!r}. "
                      f"Valid values: {[f.value for f in InvestmentFocus]}.",
        }
    ...
    demand = classify_demand(
        passenger_cagr=profile.passenger_cagr.value,
    ...
    passenger_pressure = classify_passenger_side_pressure(
        ...
        passengers_per_departure_growth=None,  # requires trend data not yet computed
    ...
    research_evidence = []  # research/ not yet wired in; empty for now
    ...
    confidence, confidence_reason = derive_confidence(
        demand, passenger_pressure, flight_pressure, critical_data_conflict=critical_conflict,
    )
    ...
    if not research:
        limitations.append(
            "Research Agent was not invoked for this assessment — "
            "Actionability defaults to Unknown unless a hard gate or clear "
            "mismatch is already evident from structured data alone."
        )

    assessment = OpportunityAssessment(
        airport=profile.airport,
        investment_focus=focus,
        objective=_FOCUS_TO_OBJECTIVE[focus],
        demand_level=demand,
        passenger_side_pressure=passenger_pressure,
        flight_side_pressure=flight_pressure,
        need_level=need,
        likely_bottleneck=likely_bottleneck,
